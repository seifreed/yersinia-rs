//! Q-in-Q (802.1ad) Protocol Implementation
...
#[async_trait]
impl Protocol for QinQProtocol {
    fn name(&self) -> &'static str {
        "Q-in-Q / 802.1ad Provider Bridging"
    }
    ...
    fn attacks(&self) -> &[AttackDescriptor] {
        static ATTACKS: [AttackDescriptor; 3] = [
            AttackDescriptor {
                id: AttackId(0),
                name: "Q-in-Q VLAN Hopping",
                description: "Hop between customer VLANs using Q-in-Q double tagging",
                parameters: vec![],
            },
            AttackDescriptor {
                id: AttackId(1),
                name: "Service VLAN Manipulation",
                description: "Manipulate Service VLAN tags to access other provider networks",
                parameters: vec![],
            },
            AttackDescriptor {
                id: AttackId(2),
                name: "Provider Bridge Bypass",
                description: "Bypass provider bridge isolation using crafted Q-in-Q frames",
                parameters: vec![],
            },
        ];
        &ATTACKS
    ...
    async fn launch_attack(
        ...
        let (attack_name, task_handle) = match attack_id.0 {
            ...
            1 => {
                let attack = ServiceVlanManipulationAttack::new(500, 600, 200);
                let name = attack.name().to_string();
                let handle = tokio::spawn(async move { attack.execute(ctx).await });
                (name, handle)
            }
            2 => {
                let attack = ProviderBridgeBypassAttack::new(100);
                let name = attack.name().to_string();
                let handle = tokio::spawn(async move { attack.execute(ctx).await });
                (name, handle)
            }
